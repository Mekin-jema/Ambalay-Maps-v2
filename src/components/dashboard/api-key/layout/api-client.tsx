...
import { DarkModeToggle } from "../../utils/toggle-theme"
import { NavUser } from "../../utils/nav-user"
...
export default function Users({ users, pagination, facetedCounts }: UsersProps) {
        ...
        <div className="mb-2 flex flex-wrap items-center justify-between space-y-2">
          ...
          <div className="flex gap-2">
            <Button variant="outline" className="space-x-1" asChild>
              <Link href={`/users/invite?${query}`}>
                <span>Invite User</span> <IconMailPlus size={18} />
              </Link>
            </Button>
            <Button className="space-x-1" asChild>
              <Link href={`/users/add?${query}`}>
